...
const schema = z.object({
  ...
  file: z
    ...
    .refine(
      (files) => ['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.openxmlformats-officedocument.presentationml.presentation'].includes(files?.[0]?.type),
      'File must be PDF, Word, or PowerPoint'
    ),
});
...
export default function NoteUploadForm() {
  ...
  useEffect(() => {
    const fetchAnalysisLimit = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/notes/analyze-limit`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        if (response.ok) {
          const data = await response.json();
          setFreeAnalysesRemaining(Math.max(0, data.freeAnalysesRemaining));
          setPurchasedAnalysesRemaining(user?.analysisPacks * 3 || 0);
        }
      } catch (err) {
        console.error('Error fetching analysis limit:', err);
      }
    };
    if (user) fetchAnalysisLimit();
  }, [user]);
  ...
  const handleAnalyze = async (data) => {
    ...
    try {
      ...
      formData.append('title', data.title || 'Analysis Request');
      ...
      if (!response.ok) {
        ...
        if (errorData.error.includes('Insufficient CNX Token')) {
      ...
      setFreeAnalysesRemaining(Math.max(0, result.freeAnalysesRemaining));
      setPurchasedAnalysesRemaining(result.purchasedAnalysesRemaining);
      await refreshUser();
      toast.success(`Analysis complete! Quality: ${result.qualityScore}/10`);
    } catch (err) {
  ...
  const onSubmit = async (data) => {
    ...
    if (analysisResult.qualityScore < 8) {
      setError('Note quality score is below 8/10. Only notes with a score of 8 or higher can be uploaded.');
      toast.error('Note quality score is below 8/10');
      return;
    }
      ...
      {analysisResult && (
        <div className="bg-blue-50 p-4 rounded-md mb-4">
          ...
          {analysisResult.evaluation && (
            <p className="text-blue-600 text-sm">Evaluation: {analysisResult.evaluation}</p>
          )}
        </div>
      )}
